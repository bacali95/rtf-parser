...
export default class RTFInterpreter extends Writable {
  doc: any;
  groupStack: any[];
  group: RTFGroup;
  hexStore: any[];
  ...
  _write(cmd, encoding, done) {
    const method =
      "cmd$" + cmd.type.replace(/-(.)/g, (_, char) => char.toUpperCase());
    if (this[method]) {
      this[method](cmd);
    }
    done();
  ...
  finisher() {
    ...
    const initialStyle = this.doc.content.length
      ? this.doc.content[0].style
      : [];
  ...
  flushHexStore() {
    if (this.hexStore.length > 0) {
      ...
      this.group.addContent(
        new RTFSpan({
          value: iconv.decode(
            Buffer.from(hexstr, "hex"),
            this.group.get("charset")
          ),
        })
      );
  ...
  cmd$controlWord(cmd) {
    ...
    const method =
      "ctrl$" + cmd.value.replace(/-(.)/g, (_, char) => char.toUpperCase());
    if (this[method]) {
  ...
  cmd$error(cmd) {
    this.emit(
      ...
      new Error(
        "Error: " +
        cmd.value +
        (cmd.row && cmd.col ? " at line " + cmd.row + ":" + cmd.col : "") +
        "."
      )
    );
  }
  ...
  ctrl$u(num) {
    const charBuf = Buffer.alloc ? Buffer.alloc(2) : new Buffer(2);
    ...
    this.group.addContent(
      new RTFSpan({value: iconv.decode(charBuf, "ucs2")})
    );
  }
  ...
  ctrl$f(num) {
    if (this.group instanceof FontTable) {
      ...
      fontCharset =
        fontCharset && fontCharset !== "ASCII"
          ? fontCharset
          : this.group.charset; // default font charset
  ...
  ctrl$fnil() {
    if (
      this.group instanceof FontTable ||
      this.group.parent instanceof FontTable
    ) {
  ...
  ctrl$fcharset(code) {
    if (
      ...
      if (charset == null) {
        return this.emit(
          "error",
          new Error("Unsupported charset code #" + code)
        );
      }
...
class FontTable extends RTFGroup {
  table: any[];
  currentFont: { charset: string; name: string; family: string };
